...
public class DanhMucRepo {
    public List<DanhMuc> getAllDM() {
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            while (rs.next()) {
                DanhMuc cl = new DanhMuc(rs.getString(1), rs.getString(2), rs.getString(3), rs.getDate(4), rs.getDate(5), rs.getBoolean(6));
                lists.add(cl);
            }
            return lists;
        } catch (Exception e) {
    ...
    public boolean insert(DanhMuc cl) {
        try {
            ...
            String sql = " INSERT INTO DanhMuc(Id, Ma, Ten, NgayTao, NgaySua, TrangThai) VALUES (?,?,?,?,?,?)";
    ...
    public boolean update(DanhMuc sp) {
        ...
        try {
            Connection conn = DBConnection.getConnection();
            String sql = "UPDATE DanhMuc SET Ten = ?, NgayTao = ?, NgaySua = ?,TrangThai = ? WHERE Ma = ? ";
            PreparedStatement ps = conn.prepareStatement(sql);
            ...
            ps.setObject(5, sp.getMaDanhMuc());
    ...
    public List<DanhMuc> TimKiem(String tenDM) {
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            List<DanhMuc> lists = new ArrayList<>();
            while (rs.next()) {
                DanhMuc pt = new DanhMuc(rs.getString(1), rs.getString(2), rs.getString(3), rs.getDate(4), rs.getDate(5), rs.getBoolean(6));
                lists.add(pt);
            }
            return lists;
        } catch (Exception e) {
