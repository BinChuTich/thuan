   ...
   public ArrayList<KhachHang> getAll() {
        try {
            ...
            while (rs.next()) {
                ...
                list.add(new KhachHang(sdt, maKH, diaChi, tenKH, diaChi, gioiTinh, email, sdt, ngaySinh, ngaySinh, trangThai, trangThai, ngaySinh, ngaySinh, trangThai));
            }
            return list;
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    try {
        Connection con = DBConnection.getConnection();
        String sql = "UPDATE KhuyenMai SET tenKhachHang = ?, sdt = ?, email = ?, diaChi = ?, gioiTinh = ?, ngaySinh = ?, trangThai = ? WHERE maKhachHang = ?";
